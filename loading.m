...
%dist=[8,16,24,32];
tot_shear=[1e-4,1e-4,2e-4,2e-4,3e-4,4e-4,5e-4];
le=[25,28,31,34,37,40,43];
i=1;
for stress=500:50:700
    
    if i>4; 
       clear vn rn dt dt0 lmin lmax a rann rntol areaminmag areamin aremax mini;
       run Input/input_inclusion_Al_3.m;
    end
    ...
    if radius == 0
        ...
        doinclusion=1;
        %inclusion_pos_rad(1)=sqrt(radius^2/2)+6;
        %inclusion_pos_rad(2)=-sqrt(radius^2/2)-6;
        %inclusion_pos_rad(3)=0;
        inclusion_pos_rad(4)=radius;
        inclusion_pos_rad(6)=(radius+0.5);
        a=0.01166066;
        b=0.14993865;
        c=a*radius+b; %0.2+radius*0.01; 
        inclusion_pos_rad(5)=c;
    end
    % Remember to change names!
    filename = sprintf('Stress_Strain_curves/Stress_Strain_curve_r%02.0f_length%03.0f_stress%03.0f.txt',radius,sqrt(2*le(i)^2+(2*le(i))^2),stress);
    videoname = sprintf('Stress_Strain_curves/dislocation_r%02.0f_length%03.0f_stress%03.0f.avi',radius,sqrt(2*le(i)^2+(2*le(i))^2),stress);
    ...
    stress_multi=stress;
    timelimit=1e-8;
    [rnnew,vn,dt,fn,fseg,totaltime]=feval(integrator,rn,dt,dt0,MU,NU,a,Ec,links,connectivity,0,rmax,rntol,mobility,dopartials,stackforcevec,totaltime,rann,doSFT,SFT_plane,doinclusion,inclusion_pos_rad,curstep,printfreq,doshielding,SurfacePlane,dobox,boxD);
    rnnew=[rnnew(:,1:3) vn rnnew(:,4)];
    if(doremesh)
        %remesh
        for i=1:5
            [rnnew,links,connectivity,linksinconnect,fseg,stackforcevec]=remesh(rnnew,links,connectivity,linksinconnect,fseg,lmin,lmax,areamin,areamax,MU,NU,a,Ec,appliedstress,mobility,dopartials,stackforcevec,rntol,doSFT,SFT_plane,doinclusion,inclusion_pos_rad,doshielding,SurfacePlane,dobox,boxD);      
        end
    end
    rn=[rnnew(:,1:3) rnnew(:,7)];
    while(timelimit>totaltime)
...
%         end  
        ...
        [ep_tot,norm_shear_strain,conv,ep_tot_conv,norm_shear_strain_conv,mini]=TotalStrain(ep_inc,glideplane,ep_tot,stress_multi,norm_shear_strain_old, totaltime,total_shear,plim,j,p_file,ep_inc_conv,wp_inc_conv,ep_tot_conv,mini,dt,eps,L_conv);
...
%         end
        ...
        frame = getframe;
        writeVideo(vidObj,frame); 
        fprintf(p_file,'%e\t%e\t%e\n',stress,norm_shear_strain,totaltime);
        %mov=addframe(mov,getframe(fig));
