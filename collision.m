function [rn,links,connectivity,linksinconnect,fseg,stackforcevec]=collision(rn,links,connectivity,linksinconnect,fseg,mindist,MU,NU,a,Ec,mobility,appliedstress,dopartials,stackforcevec,rntol,doSFT,SFT_plane,doinclusion,inclusion_pos_rad,doshielding,SurfacePlane,dobox,boxD,dt,Bt_edge,Bt_screw)
...
%                         flag=4;
%                     end
                    [rn,links,connectivity,linksinconnect,stackforcevec]=splitnode(rn,links,connectivity,linksinconnect,spnode,splitconnection,posvel,stackforcevec,flag);
...
%                     end
                ...
                [rn,connectivity,links,linksinconnect,fseg,mergednodeid,stackforcevec]=mergenodes(rn,connectivity,links,linksinconnect,fseg,mergenode1,mergenode2,MU,NU,a,Ec,mobility,dopartials,stackforcevec,inclusion_pos_rad);
                if mergednodeid>0
                    for k=1:connectivity(mergednodeid,1)
                        linkid=connectivity(mergednodeid,2*k);
                        fseg=segforcevec(MU,NU,a,Ec,rn(:,[1 2 3 lrn2]),links,appliedstress,linkid,mobility,dopartials,stackforcevec,doinclusion,inclusion_pos_rad,doSFT,SFT_plane,doshielding,SurfacePlane,dobox,boxD);
                        ...
                        [rn(othernode,4:6),fntmp]=feval(mobility,fseg,rn,links,connectivity,othernode,clist,mobility,dopartials,stackforcevec,rntol,doSFT,SFT_plane,doinclusion,inclusion_pos_rad,dt,Bt_edge,Bt_screw);
                    end
                    numbcon=connectivity(mergednodeid,1);
                    conlist=[numbcon linspace(1,numbcon,numbcon)];
                    [rn(mergednodeid,4:6),fntmp]=feval(mobility,fseg,rn,links,connectivity,mergednodeid,conlist,mobility,dopartials,stackforcevec,rntol,doSFT,SFT_plane,doinclusion,inclusion_pos_rad,dt,Bt_edge,Bt_screw);
                end
            end
        end
        j=j+1;
    ...
    while j<=connectivity(i,1)
        ...
        while k<=connectivity(i,1)
            ...
            if j~=k
                ...
                len1=norm(rn(nodenoti,1:3)-rn(i,1:3));
                len2=norm(rn(n2s1,1:3)-rn(n1s1,1:3));
                tang1=rn(nodenoti,1:3)-rn(i,1:3);
                if(n1s1==i)
                    tang2=rn(n2s1,1:3)-rn(n1s1,1:3);
                else
                    tang2=rn(n1s1,1:3)-rn(n2s1,1:3);
                end
                angbetseg=acos(dot(tang1,tang2)/(norm(tang1)*norm(tang2)));
                angref=asin(mindist/lref);
                collision_condition_is_met=((dist2<mindist2)&(ddist2dt<-eps));%|((angbetseg<=angref)&(len1<=len2));
                if collision_condition_is_met
                    % identify the first node to be merged
                    mergenode1=nodenoti;
                    % identify the second node to be merged
                    vec=rn(n1s1,1:lrn3)-rn(n2s1,1:lrn3);
                    close_to_n1s1=((L1*L1*(vec*vec'))<mindist2);
                    close_to_n2s1=(((1-L1)*(1-L1)*(vec*vec'))<mindist2);
                    % if collision point is close to one of the existing nodes use that node
                    if close_to_n1s1
...
ddist2dt=2*((vx0+vseg1.*L1-vy0-vseg2.*L2)*(x0+seg1.*L1-y0-seg2.*L2)');
 ...
 v1=rn(mergenode1,4:6);
 v2=rn(mergenode2,4:6);
 norm_v1=norm(v1);
 norm_v2=norm(v2);
 min_v=min(norm_v1,norm_v2);
 max_v=max(norm_v1,norm_v2);
 Nmat=zeros(3,3);
 Nsize=0;
 vector=zeros(3,1);
 s=size(rn,2);
 
 if (rn(mergenode1,s)==7 || rn(mergenode1,s)==4 || rn(mergenode1,s)==-4 || rn(mergenode1,s)==3)
     collisionpoint=rn(mergenode1,1:3);
     return;
 elseif (rn(mergenode2,s)==7 || rn(mergenode2,s)==4 || rn(mergenode2,s)==-4 || rn(mergenode2,s)==3)
