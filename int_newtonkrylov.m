function [rnnew,vn,dt,fn,fseg,totaltime]=int_newtonkrylov(rn,dt,dt0,MU,NU,a,Ec,links,connectivity,appliedstress,rmax,rntol,mobility,dopartials,stackforcevec,totaltime)
...
ierr=5;
tol=[rntol 1e-2];
[vnvec,fn,fseg]=drndt(t,rnvec,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec);
while ierr~=0
    dt=0.5*dt;
    [sol, it_hist, ierr, x_hist] = nkiter(rnvec,'calposerr',tol, parms,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec);
    
end 
rnnew=[reshape(sol,length(sol)/3,3),flag];
vn=reshape(vnvec,length(vnvec)/3,3);
if(dt==dt1 & size(it_hist,1)<4)
    dt=min(dt1*1.2,dt0);
end
totaltime=totaltime+dt;

function [sol, it_hist, ierr, x_hist] = nkiter(x,f,tol,parms,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec)
...
%
f0 = feval(f,x,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec);
fnrm = norm(f0);
...
%
    ...
    [step, errstep, inner_it_count,inner_f_evals] = ...
         dkrylov(f0, f, x, gmparms, lmeth,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec);
...
%
    ...
    xt = x + lambda*step;
    ft = feval(f,xt,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec);
...
function [step, errstep, total_iters, f_evals] = ...
    dkrylov(f0, f, x, params, lmeth,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec)
...
% for this purpose
%
    [step, errstep, total_iters] = dgmres(f0, f, x, gmparms,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec);
    kinn = 0;
%
...
%
    while total_iters == lmaxit & ...
          ...
          kinn < restart_limit
        kinn = kinn+1;
        [step, errstep, total_iters] = dgmres(f0, f, x, gmparms,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec,step);
    end
...
%
%
function z = dirder(x,w,f,f0,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec)
...
%
del = x+epsnew*w;
f1 = feval(f,del,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec);
z = (f1 - f0)/epsnew;
%
%
function [x, error, total_iters] = dgmres(f0, f, xc, params, rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec,xinit)
...
if nargin == 5
    x = xinit;
    r = -dirder(xc, x, f, f0,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility)-f0;
end
%
%
h = zeros(kmax);
...
%
    v(:,k+1) = dirder(xc, v(:,k), f, f0,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec);
    normav = norm(v(:,k+1));
%
...
while((zeta > errtol) & (k < kmax))
    ...
    p = r+beta*(p - omega*v);
    v = dirder(xc,p,f,f0,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
    tau = hatr0'*v;
    if tau == 0
    ...
    alpha = rho(k+1)/tau;
    s = r-alpha*v; 
    t = dirder(xc,s,f,f0,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
    tau = t'*t;
    if tau == 0
...
k = 0; d = zeros(n,1); 
v = dirder(xc, y(:,1),f,f0,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
u(:,1) = v;
...
%
        if j == 2 
            y(:,2) = y(:,1)-alpha*v;
            u(:,2) = dirder(xc, y(:,2),f,f0,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
        end
        m = 2*k-2+j;
        w = w-alpha*u(:,j);
...
%
    ...
    y(:,1) = w + beta*y(:,2);
    u(:,1) = dirder(xc, y(:,1),f,f0,rnvec,vnvec,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility);
...
%
%
function [pos_error]=calposerr(rnvec1,rnvec0,vnvec0,dt,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec);

    [vnvec,fn,fseg]=drndt(dt,rnvec1,flag,MU,NU,a,Ec,links,connectivity,appliedstress,mobility,dopartials,stackforcevec);
    pos_error=rnvec1-rnvec0-(vnvec+vnvec0)*0.5*dt;