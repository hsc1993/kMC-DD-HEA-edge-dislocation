function [fseg]=segforcevec(MU,NU,a,Ec,rn,links,sigext,linkid,mobility,dopartials,stackforcevec,doinclusion,inclusion_pos_rad,doSFT,SFT_plane,doshielding,SurfacePlane,dobox,boxD)
...
function [finclusion_1, finclusion_2]=ForceInsideInclusion(MU,NU,B_2,R_i,MU_i,NU_i,rho_i,B_i,alpha_i,beta_i,gamma_i,delta_i,b1_newbase,x1_newbase,x2_newbase,d,H_j,L_j)
    ...
    %f_2_2(1)=-(d/L_j)*f_total_1(1) - (b2/L_j)*((gamma_i/(6*R_i^3))*((z_2^6 - z_1^6) + 3*x^2*(z_2^4 - z_1^4) + 3*x^4*(z_2^2 -z_1^2)) - (delta_i/3)*(norm_r2^3 - norm_r1^3));
    %f_2_3(1)=-(d/L_j)*f_total_1(1) - b3*(H_j/L_j)*((alpha_i/R_i^3)*((z_2^5 - z_1^5)/5 + (x^2/3)*(z_2^3 - z_1^3)) - (beta_i/2)*((z_2*norm_r2 - z_1*norm_r1) - x^2*log((z_2+sqrt(x^2+z_2^2))/(z_1+sqrt(x^2+z_1^2)))));
