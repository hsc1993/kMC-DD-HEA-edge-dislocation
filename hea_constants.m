function [a_alloy,Bt_edge_GPa,Bt_screw_GPa,mu_v,edgecrss,screwcrss] = hea_constants(T,flag_plot)
...
screw_Bt_vs_T_fitting_coefficients = polyfit(screw_sampling_temperature',screwfriction_coefficient_gpa, 2);

xdata = screw_sampling_temperature
ydata = screwfriction_coefficient_gpa
fun = @(x,xdata) x(1)*log(x(2)*xdata);
x0 = [screw_sampling_temperature(1),screwfriction_coefficient_gpa(1)]
screw_Bt_log_fit_params = lsqcurvefit(fun,x0,xdata,ydata)
...
%         add legends
    ...
    Bt_screw_GPa = screw_Bt_vs_T_fitting_coefficients(1)*T.^2+screw_Bt_vs_T_fitting_coefficients(2)*T+screw_Bt_vs_T_fitting_coefficients(3);
    subplot(2,3,5)
    hold on
    ...
    plot(screw_sampling_temperature,screw_Bt_vs_T_fitting_coefficients(1)*screw_sampling_temperature.^2+screw_Bt_vs_T_fitting_coefficients(2)*screw_sampling_temperature+screw_Bt_vs_T_fitting_coefficients(3))
    
    screw_Bt_log = fun(screw_Bt_log_fit_params,screw_sampling_temperature)
    plot(screw_sampling_temperature,screw_Bt_log)
    scatter(T,Bt_screw_GPa,100,'red','filled')
    hold off
    xlabel('Temperature [K]', 'FontSize', 20)
    ylabel('Screw Bt [GPa*s]', 'FontSize', 20)
    
end
    
    Bt_edge_GPa = edgefriction_coefficient_gpa;
    mu_v = debye_frequency_alloy;
    % need T as entry
    edgecrss = edgecrss_coefficient(1)*T+edgecrss_coefficient(2);    % in GPa
    screwcrss = screwcrss_coefficient(1)*T+screwcrss_coefficient(2); % in GPa
