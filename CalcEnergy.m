function [W]=CalcEnergy(rn,links,mu,nu,a,Ec,dobox,boxD)
...
% function [W]=CalcEnergy(rn,links,mu,nu,a,Ec)
    ...
    W=0.0;
    L=size(links,1);
    Wself=SegSelfEnergy(mu,nu,Ec,a,rn(links(:,1),1:3),rn(links(:,2),1:3),links(:,3:5),links(:,6:8),dobox,boxD);
    ...
    Winteract=SegSegInteractionEnergy(rn(index1,1:3),rn(index2,1:3),rn(index3,1:3),...
        rn(index4,1:3),links(bpindex,3:5),links(bindex,3:5),a,mu,nu,dobox,boxD);
    W=sum(Wself)+sum(abs(Winteract));



    
function [W]=SegSelfEnergy(mu,nu,Ec,a,x1,x2,b,n,dobox,boxD)
...
numseg=size(x1,1);
inbox=ones(size(x1,1),1);
for i=1:numseg
    normburgseg=norm(b(i,:));
    glideplane=n(i);
    ...
    if(dobox)
        if((x1(i,1)>(boxD(1,2)+TOL) || x1(i,1)<(boxD(1,1)-TOL)) || ...
                (x1(i,2)>(boxD(2,2)+TOL) || x1(i,2)<(boxD(2,1)-TOL)) || ...
                (x1(i,3)>(boxD(3,2)+TOL) || x1(i,3)<(boxD(3,1)-TOL)) || ...
                (x2(i,1)>(boxD(1,2)+TOL) || x2(i,1)<(boxD(1,1)-TOL)) || ...
                (x2(i,2)>(boxD(2,2)+TOL) || x2(i,2)<(boxD(2,1)-TOL)) || ...
                (x2(i,3)>(boxD(3,2)+TOL) || x2(i,3)<(boxD(3,1)-TOL)))
            inbox(i) = 0;
        end
    end
end
...
for i=1:size(x1,1)
    if(L(i)>eps)
        Linv=1./L(i);
        t=Diff(i,:).*[Linv Linv Linv];

        bs=sum(b(i,:).*t,2);
        bs2=bs.*bs;
        be2=sum(b(i,:).*b(i,:),2)-bs2;

        La=sqrt(L(i).*L(i)+a*a);
        common=2.*L(i).*log((La+L(i))./a);
    %Self Elastic Energy
        W(i)=(0.125*mu/pi) .*(  be2.*common./(1-nu) + bs2.*(common -((3-nu)/(1-nu)).*(La - a) ));
    %Self Core energy
        W(i)=W(i)+(bs2 + be2./(1-nu)).*Ecvec(i)'.*L(i);
        W(i)=W(i).*inbox(i);
    end
end
...
function [W12]=SegSegInteractionEnergy(x1,x2,x3,x4,bp,b,a,mu,nu,dobox,boxD)
...
%             bp(i,:) = 0;
%         end
% %         if((x3(i,1)>(boxD(1,2)+TOL) || x3(i,1)<(boxD(1,1)-TOL)) || ...
% %             (x3(i,2)>(boxD(2,2)+TOL) || x3(i,2)<(boxD(2,1)-TOL)) || ...
% %             (x3(i,3)>(boxD(3,2)+TOL) || x3(i,3)<(boxD(3,1)-TOL)) || ...
% %             (x4(i,1)>(boxD(1,2)+TOL) || x4(i,1)<(boxD(1,1)-TOL)) || ...
% %             (x4(i,2)>(boxD(2,2)+TOL) || x4(i,2)<(boxD(2,1)-TOL)) || ...
% %             (x4(i,3)>(boxD(3,2)+TOL) || x4(i,3)<(boxD(3,1)-TOL)))
% %             b(i,:) = 0;
% %         end
%     end
% end
...
DiffLp=x2-x1;
L=sqrt(sum(Diff.*Diff,2));
Lp=sqrt(sum(DiffLp.*DiffLp,2));

oneoverL=1./L;
t=Diff.*[oneoverL oneoverL oneoverL];
oneoverLp=1./Lp;
tp=DiffLp.*[oneoverLp oneoverLp oneoverLp];
...
if sL > 0
    ...
    [W12sp]=SpecialSegSegInteractionEnergy(x1sp,x2sp,x3sp,x4sp,bpsp,bsp,a,mu,nu,eps,dobox,boxD);
    xL=size(x1,1);
    for k=1:sL
        index=sL+1-k;
        pos=spindex(index);
...
function [W12]=SpecialSegSegInteractionEnergy(x1,x2,x3,x4,bp,b,a,mu,nu,ecrit,dobox,boxD);
...
if corsize>0
    W12corr=SegSegInteractionEnergy(x12,x1mod2,x32,x42,bp2,b2,a,mu,nu,dobox,boxD);
    W12corr=W12corr+SegSegInteractionEnergy(x2mod2,x22,x32,x42,bp2,b2,a,mu,nu,dobox,boxD);
    for i=1:corsize
        index=corindex(i);
