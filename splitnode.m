function [rn,links,connectivity,linksinconnect,stackforcevec]=splitnode(rn,links,connectivity,linksinconnect,nodeid,splitconnections,posvel,stackforcevec,flag)
...
stackv=stackforcevec(connectivity(newnodeid,2),:);%Stacking fault forces
...
for i=2:connectivity(newnodeid,1)
    if posnew+connectivity(newnodeid,2*i+1)==3
        ...
        stackv=stackv-stackforcevec(connectivity(newnodeid,2*i),:);%THAT IS THE KEY
    else
        burgv=burgv+links(connectivity(newnodeid,2*i),3:5);
        stackv=stackv+stackforcevec(connectivity(newnodeid,2*i),:);%THAT IS THE KEYYYYYYYYYY
    end
end
if (norm(burgv)>eps)
    ...
    if(connectivity(newnodeid,1)==2)
        links(lastlink,6:8)=links(connectivity(newnodeid,2),6:8)/norm(links(connectivity(newnodeid,2),6:8));
    elseif(connectivity(nodeid,1)==2)
        links(lastlink,6:8)=links(connectivity(nodeid,2),6:8)/norm(links(connectivity(nodeid,2),6:8));
    elseif n1*n1'>eps
