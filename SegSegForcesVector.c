...
static void SpecialSegSegForce(double p1x, double p1y, double p1z,
 ...
 void mexFunction(
    ...
    double *f1a,*f2a,*f3a,*f4a;
    double *df1dxa, *df2dxa;
    
    /* Check for proper number of input arguments. */
    if (nrhs!=11) mexErrMsgTxt("11 inputs required!");
    ...
    df1dxa=mxGetPr(prhs[4]);
    df2dxa=mxGetPr(prhs[5]);
    ...
    seg12Local=mxGetScalar(prhs[9]);
    seg34Local=mxGetScalar(prhs[10]);
     ...
     for (i=0; i<4; i++)  
     {
	     plhs[i]=mxCreateDoubleMatrix(3,1,mxREAL);
	     plhs[i+4]=mxCreateDoubleMatrix(3,12,mxREAL);
	 }
...
static void SpecialSegSegForceHalf(double p1x, double p1y, double p1z,
                                double p2x, double p2y, double p2z,
                                double p3x, double p3y, double p3z,
                                double p4x, double p4y, double p4z,
                                double bpx, double bpy, double bpz,
...
{                                
        ...
        double eps, c, a2, d2, a2_d2, a2d2inv;
                        ...
                        double *fp4x, double *fp4y, double *fp4z)
{
        ...
        double temp1, temp2, temp3, temp4[4], tmp[10];
        double yv[4], zv[4], y2[4], z2[4], Ra[4], Rainv[4];
        double Ra_Rdot_tp[4], Ra_Rdot_t[4], log_Ra_Rdot_tp[4], log_Ra_Rdot_t[4];
        ...
        double tcbp[3], tctpcbpct[3], tctpcbp[3], tctpct[3], tpct[3];
        double tctpcbpdb, tctpcbpdtp, tctpdb, tdb, tdbp;
        double tpcbctp[3], tpctcb[3], tpctctp[3], tpcb[3], tpctcbctp[3];
