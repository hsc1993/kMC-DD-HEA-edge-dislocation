function [rn,connectivity,links,linksinconnect,fseg,nodeid,stackforcevec]=mergenodes(rn,connectivity,links,linksinconnect,fseg,nodeid,deadnode,MU,NU,a,Ec,mobility,dopartials,stackforcevec,inclusion_pos_rad)
...
while i<connectivity(nodeid,1)
    ...
    if nodeid==nodenoti_linkid
        [links,connectivity,linksinconnect,fseg,stackforcevec]=removelink(links,connectivity,linksinconnect,fseg,linkid,mobility,dopartials,stackforcevec);
    else
        i=i+1; 
    end
end
    ...
    while j<=connectivity(nodeid,1)
        ...
        if nodenoti_linkid==nodenoti_linkid2
            ...
            if posi_linkid+posi_linkid2==3
                ...
                stackforcevec(linkid,:)=stackforcevec(linkid,:)-stackforcevec(linkid2,:);%THAT IS GREAT!!!!
            else %posi_linkid+posi_linkid2~=3
                links(linkid,3:5)=links(linkid,3:5)+links(linkid2,3:5);
                stackforcevec(linkid,:)=stackforcevec(linkid,:)+stackforcevec(linkid2,:);%%%%%%%
            ...
            if n1*n1'>eps
                ...
                if(findstr(mobility,'fcc'))
                    links(linkid,6:8)=AssignGlidePlaneScrewfcc(n2);
                elseif(n2*n2'>eps)
            ...
            [links,connectivity,linksinconnect,fseg,stackforcevec]=removelink(links,connectivity,linksinconnect,fseg,linkid2,mobility,dopartials,stackforcevec);
...
function [links,connectivity,linksinconnect,fseg,stackforcevec]=removelink(links,connectivity,linksinconnect,fseg,linkid,mobility,dopartials,stackforcevec);
...
[links,connectivity,linksinconnect,fseg,stackforcevec]=removedeadlink(links,connectivity,linksinconnect,fseg,linkid,mobility,dopartials,stackforcevec);

function [linksnew,connectivitynew,linksinconnectnew,fsegnew,stackforcevecnew]=removedeadlink(linksnew,connectivitynew,linksinconnectnew,fsegnew,deadlink,mobility,dopartials,stackforcevecnew);
% this subroutine is called by meshcoarsen
...
if deadlink<llinks
    ...
    %For partials dislocations
    if(findstr(mobility,'fcc')&(dopartials))
        stackforcevecnew(deadlink,:)=stackforcevecnew(llinks,:);
    end
end
linksnew(llinks,:)=[];
fsegnew(llinks,:)=[];
